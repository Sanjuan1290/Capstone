...
const addInventoryBatch = async (
  inventoryId,
  { quantity, expiration_date, note = null, batch_code = null, unit_cost = 0, location = MAIN_LOCATION, location_id = null },
  executor = db
) => {
  ...
  const [result] = await executor.query(
    `INSERT INTO inventory_batches (inventory_id, quantity, expiration_date, note, batch_code, unit_cost)
     VALUES (?,?,?,?,?,?)`,
    [inventoryId, batchQty, normalizeExpiryDate(expiration_date), note || null, normalizeBatchCode(batch_code), Math.max(0, Number(unit_cost) || 0)]
  )
...
const receiveInventoryBatch = async (
  ...
  if (existingBatchId > 0) {
    const [[batch]] = await executor.query(
      `SELECT id, inventory_id, batch_code, quantity, expiration_date, unit_cost, archived_at
       FROM inventory_batches
    ...
    if (batch.archived_at) throw Object.assign(new Error('Archived batches cannot receive stock. Restore the batch first.'), { statusCode: 409, code: 'BATCH_ARCHIVED' })
...
const attachBatchesToInventory = async (items, executor = db) => {
  ...
  const [rows] = await executor.query(
    `SELECT b.id, b.inventory_id, b.batch_code, b.quantity, b.expiration_date, b.received_at, b.note, b.unit_cost, b.archived_at, b.archived_by_admin_id, b.archive_reason,
    ...
    return executor.query(
      `SELECT id, inventory_id, NULL AS batch_code, quantity, expiration_date, received_at, note, unit_cost, archived_at, archived_by_admin_id, archive_reason,
