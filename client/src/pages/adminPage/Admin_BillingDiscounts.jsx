...
const Admin_BillingDiscounts = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setRows(Array.isArray(discounts) ? discounts : []); setPendingCount(Array.isArray(pending) ? pending.length : 0)
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-5">
    ...
    <Modal open={Boolean(editor)} onClose={() => !saving && setEditor(null)} closeDisabled={saving} title={editor?.id ? 'Edit Discount' : 'Add Discount'} description="Keep this form short; complex billing setup stays on dedicated pages." size="md"><div className="space-y-4"><label className="block"><span className="form-label">Name *</span><input className="form-control mt-1.5" value={form.label} onChange={(e) => update('label', e.target.value)} placeholder="e.g. Senior Citizen" /></label><div className="grid gap-4 sm:grid-cols-2"><label><span className="form-label">Type</span><select className="form-control mt-1.5" value={form.discount_type} onChange={(e) => update('discount_type', e.target.value)}><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option></select></label><label><span className="form-label">Default Value</span><input type="number" min="0" step="0.01" className="form-control mt-1.5" value={form.value} onChange={(e) => update('value', e.target.value)} /><span className="form-helper">Use 0 when Staff should enter the amount during review.</span></label></div><div className="space-y-2 rounded-2xl bg-slate-50 p-4">{[['requires_reference','Require ID / reference'],['requires_admin_approval','Require Admin approval'],['is_active','Active and available at checkout']].map(([key,label]) => <label key={key} className="flex items-center gap-3 text-sm font-semibold text-slate-700"><input type="checkbox" checked={Boolean(form[key])} onChange={(e) => update(key, e.target.checked)} /> {label}</label>)}</div><label className="block"><span className="form-label">Sort Order</span><input type="number" className="form-control mt-1.5" value={form.sort_order} onChange={(e) => update('sort_order', e.target.value)} /></label><div className="flex justify-end gap-2"><button className="button-secondary" disabled={saving} onClick={() => setEditor(null)}>Cancel</button><button className="button-primary" disabled={saving || !form.label.trim()} onClick={save}>{saving ? 'Saving…' : 'Save Discount'}</button></div></div></Modal>
  </div>
}
