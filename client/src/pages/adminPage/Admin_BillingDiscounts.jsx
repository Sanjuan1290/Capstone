...
import { getDiscountPresets, saveDiscountPreset } from '../../services/admin.service'
...
const blank = { label: '', discount_type: 'percentage', value: 0, requires_reference: true, requires_admin_approval: false, is_active: true }
...
const Admin_BillingDiscounts = () => {
  ...
  const load = async () => {
    ...
    try {
      const discounts = await getDiscountPresets()
      setRows(Array.isArray(discounts) ? discounts : [])
  ...
  const openEdit = (row) => { setEditor(row); setForm({ label: row.label || '', discount_type: row.discount_type || 'fixed', value: Number(row.value || 0), requires_reference: asBool(row.requires_reference), requires_admin_approval: asBool(row.requires_admin_approval), is_active: asBool(row.is_active) }) }
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900"><MdDiscount className="text-amber-500" /> Discounts</h1><p className="mt-1 text-sm text-slate-500">Control reusable discount rules, required references, and Admin approval.</p></div><div className="flex gap-2"><button className="button-secondary" onClick={load}><MdRefresh /> Refresh</button><button className="button-primary" onClick={openNew}><MdAdd /> Add Discount</button></div></div><BillingSetupNav />
