...
const Admin_BillingCatalog = () => {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900"><MdPayments className="text-amber-500" /> Services & Pricing</h1><p className="mt-1 text-sm text-slate-500">Manage the actual services patients can book and Doctors can select, including pricing and default consumables.</p></div><div className="flex gap-2"><button className="button-secondary" onClick={load}><MdRefresh /> Refresh</button><Link className="button-primary" to={`${portalBase}/system-setup/billing/services/new`}><MdAdd /> Add Service</Link></div></div><BillingSetupNav />
