...
const AddModal = ({ onClose, onAdd }) => {
  ...
  const valid = form.full_name.trim() && form.email.trim() && form.password.trim()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div><p className="text-sm font-bold text-slate-800">Add Staff Account</p><p className="text-xs text-slate-500 mt-0.5">Credentials will be shared with the staff</p></div>
...
const Admin_StaffAccount = () => {
  ...
  const filtered = staff.filter(s => {
    ...
    const matchSearch = !search || name.toLowerCase().includes(search.toLowerCase()) || s.email.toLowerCase().includes(search.toLowerCase())
    ...
    <div className="max-w-5xl space-y-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <button onClick={() => setShowAdd(true)}
          className="flex items-center gap-1.5 bg-[#0b1a2c] hover:bg-[#122236] text-white text-xs font-semibold px-4 py-2.5 rounded-xl transition-all shadow-md active:scale-95 shrink-0">
