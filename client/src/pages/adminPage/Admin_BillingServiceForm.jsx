...
const Admin_BillingServiceForm = () => {
  ...
  const openCategorySetup = () => { if (dirty && !window.confirm('You have unsaved service changes. Leave this page and manage Service Categories?')) return; navigate('/admin/system-setup?tab=service_categories') }
  ...
  const remove = async () => { setDeleting(true); try { const result = await deleteBillingCatalogService(serviceId); toast.success(result?.message || 'Service removed.'); setDirty(false); navigate('/admin/billing/setup/services') } catch (err) { toast.error(err.message || 'Service could not be removed.') } finally { setDeleting(false) } }
  const leaveEditor = () => { if (dirty && !window.confirm('You have unsaved service changes. Leave this page and discard them?')) return; navigate('/admin/billing/setup/services') }
