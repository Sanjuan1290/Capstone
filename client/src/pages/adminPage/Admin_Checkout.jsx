...
const Admin_Checkout = ({ embedded = false }) => {
  const toast = useToast()
  const [filter,setFilter]=useState('draft,pending'),[search,setSearch]=useState(''),[page,setPage]=useState(1),[pageSize,setPageSize]=useState(10)
  const [data,setData]=useState({items:[],summary:{},pagination:{page:1,total:0,totalPages:1}}),[loading,setLoading]=useState(true),[error,setError]=useState('')
  const load=async()=>{setLoading(true);setError('');try{setData(await getAdminCheckoutBills({status:filter,search,page,limit:pageSize}))}catch(err){setError(err.message||'Checkout queue could not be loaded.');toast.error(err.message||'Checkout queue could not be loaded.')}finally{setLoading(false)}}
  ...
  return <div className={`${embedded ? 'w-full' : 'mx-auto w-full max-w-6xl'} space-y-5`}>
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      {loading?<div className="p-5"><LoadingState label="Loading checkout queue..."/></div>:error?<div className="p-5"><ErrorState message={error} onRetry={load}/></div>:!items.length?<div className="p-5"><EmptyState title="Nothing in this queue" description="No bills match this checkout stage."/></div>:<div className="divide-y divide-slate-100">{items.map(bill=><Link key={bill.id} to={`/admin/billing/checkout/${bill.id}`} className="block p-5 hover:bg-slate-50"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-black text-slate-900">{bill.patient_name}</h2><BillingStatusBadge status={bill.status}/></div><p className="mt-1 text-sm text-slate-500">{bill.doctor_name} · {bill.appointment_reason||'Consultation'}</p><p className="mt-2 text-xs font-semibold text-slate-400">{bill.appointment_date||'—'} · {bill.appointment_time||'—'} · Bill #{bill.id}</p></div><div className="text-right"><p className="text-xs font-bold uppercase text-slate-400">{bill.status==='partially_paid'?'Balance':bill.status==='paid'?'Paid':'Amount'}</p><p className="mt-1 text-xl font-black text-slate-900">{formatMoney(bill.status==='partially_paid'?bill.balance_amount:bill.total_amount)}</p><p className="mt-2 text-xs font-bold text-amber-700">Open Checkout →</p></div></div></Link>)}</div>}
