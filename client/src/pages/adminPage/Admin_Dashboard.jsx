...
const Admin_Dashboard = () => {
  ...
  const load = useCallback(() => {
    Promise.all([getDashboard(), getAppointments(`?date=${todayISO}`), getSupplyRequests()])
  ...
  const handleResolve = async (id, status) => {
    try { await resolveSupplyRequest(id, status); setPendingRequests(p=>p.filter(r=>r.id!==id)) }
    ...
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-[#0b1a2c] px-7 py-6 shadow-md">
        ...
        <div className="relative flex items-start justify-between gap-4">
          ...
          <button onClick={load} title="Refresh now" className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/10 hover:bg-white/20 text-white transition-colors shrink-0 mt-1">
        ...
        {stats.map(({label,value,change,up,color,bg,border}) => (
          <div key={label} className={`${bg} border ${border} rounded-2xl px-5 py-4 transition-transform hover:scale-[1.02]`}>
            ...
            <div className="flex items-center gap-1 mt-0.5">
              ...
              <p className={`text-[11px] font-medium ${up===true?'text-emerald-600':up===false?'text-red-500':'text-slate-400'}`}>{change}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <NavLink to="/admin/appointments" className="text-xs font-semibold text-amber-600 hover:text-amber-700 flex items-center gap-0.5">View all <MdChevronRight className="text-[14px]" /></NavLink>
          </div>
          <div className="divide-y divide-slate-100">
            {todayAppts.length>0 ? todayAppts.map(appt => {
              ...
              return (
                <div key={appt.id} className={`flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50 ${appt.status==='in-progress'?'bg-violet-50/40':''}`}>
                  ...
                  <span className="text-xs text-slate-400 font-medium flex items-center gap-1 shrink-0"><MdAccessTime className="text-[12px]" />{appt.time||appt.appointment_time}</span>
                  <span className={`text-[10px] font-bold border px-2 py-0.5 rounded-full shrink-0 ${cfg.badge}`}>{cfg.label}</span>
                </div>
              )
            }) : <p className="p-10 text-center text-xs text-slate-400">No appointments today.</p>}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              ...
              <NavLink to="/admin/doctor-schedules" className="text-xs font-semibold text-amber-600 hover:text-amber-700 flex items-center gap-0.5">Schedules <MdChevronRight className="text-[14px]" /></NavLink>
            </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            <NavLink key={path} to={path} className={`flex flex-col items-center gap-2 p-4 rounded-xl ${bg} hover:opacity-80 transition-all hover:-translate-y-1 shadow-sm`}>
