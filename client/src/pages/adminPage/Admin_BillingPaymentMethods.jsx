...
const ProviderCard = ({ provider, label, enabled, setEnabled, form, setForm, uploading, status, onUpload, onRemove, onDirty }) => {
  const urlKey = `${provider}_qr_url`
  const scanKey = `${provider}_qr_scan_status`
  const modeKey = `${provider}_qr_mode`
  const scanStatus = form[scanKey]
  const mode = form[modeKey] || 'uploaded'
  const okay = scanStatus === 'approved'
  const setMode = (value) => { setForm((current) => ({ ...current, [modeKey]: value })); onDirty?.() }
  return <section className={`rounded-3xl border bg-white p-5 shadow-sm ${enabled ? 'border-slate-200' : 'border-slate-100 opacity-75'}`}>
    ...
    {enabled && <div className="mt-5 space-y-4">
      <div><p className="form-label">Payment Instructions</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => setMode('uploaded')} className={`rounded-xl border p-3 text-left text-sm font-bold ${mode === 'uploaded' ? 'border-sky-300 bg-sky-50 text-sky-800' : 'border-slate-200'}`}>Display uploaded QR in Checkout</button><button type="button" onClick={() => setMode('external')} className={`rounded-xl border p-3 text-left text-sm font-bold ${mode === 'external' ? 'border-sky-300 bg-sky-50 text-sky-800' : 'border-slate-200'}`}>Use physical QR at cashier</button></div></div>
      {mode === 'external' ? <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-700"><strong>Physical QR mode.</strong> Staff will be instructed to use the clinic's {label} QR available at the cashier. No uploaded image is required.</div> : <div className="grid gap-4 md:grid-cols-[180px_1fr]">
...
const Admin_BillingPaymentMethods = () => {
  ...
  const applyResult = (provider, result) => { setForm((current) => ({ ...current, [`${provider}_qr_url`]: result.url, [`${provider}_qr_scan_status`]: result.scan_status, [`${provider}_qr_security_token`]: result.security_token || '' })); setDirty(true) }
  ...
  const bypassScan = async () => {
    ...
    if (!window.confirm('Use an unverified QR image? Only continue if this file came directly from a trusted clinic-controlled source. This security bypass will be recorded in Audit Logs.')) return
  ...
  const removeQr = (provider) => { setForm((current) => ({ ...current, [`${provider}_qr_url`]: '', [`${provider}_qr_scan_status`]: 'legacy', [`${provider}_qr_security_token`]: '' })); setDirty(true) }

  const save = async () => {
    if (![form.cash_enabled, form.gcash_enabled, form.maya_enabled, form.bank_transfer_enabled].some(Boolean)) return toast.error('At least one payment method must remain enabled.')
    if (form.bank_transfer_enabled && (!String(form.bank_name || '').trim() || !String(form.bank_account_name || '').trim() || !String(form.bank_account_number || '').trim())) return toast.error('Complete all Bank Transfer account details before enabling it.')
    if (form.gcash_enabled && form.gcash_qr_mode !== 'external' && !form.gcash_qr_url) return toast.error('Upload a GCash QR or choose physical QR mode.')
    if (form.maya_enabled && form.maya_qr_mode !== 'external' && !form.maya_qr_url) return toast.error('Upload a Maya QR or choose physical QR mode.')
    setSaving(true)
    try {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-5">
    ...
    {loading ? <LoadingState label="Loading payment methods..." /> : error ? <ErrorState message={error} onRetry={load} /> : <>
      ...
      <div className="sticky bottom-4 z-10 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-lg backdrop-blur"><span className={`text-sm font-bold ${dirty ? 'text-amber-700' : 'text-slate-400'}`}>{dirty ? '● Unsaved changes' : 'All changes saved'}</span><div className="flex gap-2"><button className="button-secondary" disabled={!dirty || saving} onClick={load}>Discard Changes</button><button className="button-primary min-w-40" disabled={saving || Boolean(uploading) || !dirty} onClick={save}>{saving ? 'Saving…' : 'Save Changes'}</button></div></div>
    </>}
  </div>
}
