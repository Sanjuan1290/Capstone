...
const Admin_BillingTransactions = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [bills, pending] = await Promise.all([
        getBills({ search, status, payment_method: paymentMethod, date_from: dateFrom, date_to: dateTo, page, limit: pageSize }),
  ...
  }, [dateFrom, dateTo, page, pageSize, paymentMethod, search, status, toast])
  ...
  }, [dateFrom, dateTo, page, paymentMethod, search, setParams, status])

  const clearFilters = () => { setSearch(''); setStatus(''); setPaymentMethod(''); setDateFrom(''); setDateTo(''); setPage(1) }
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-5">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        {(search || status || paymentMethod || dateFrom || dateTo) && <div className="mt-3 flex justify-end"><button type="button" className="text-xs font-bold text-slate-500 hover:text-slate-800" onClick={clearFilters}>Clear filters</button></div>}
      </section>
