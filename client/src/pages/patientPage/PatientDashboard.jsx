...
import { getMyAppointments } from "../../services/patient.service" // ← added
import {
...
// ── Mock Data (kept for reference, now replaced by fetched data) ─────────────────
...
const PatientDashboard = () => {
  const { user } = useAuth()
  const [upcomingAppointment, setUpcomingAppointment] = useState(null)      // ← replaced mock
  const [recentVisits, setRecentVisits] = useState([])                     // ← replaced mock
  ...
  // For the waiting time tracker (if you keep this UI, you may later also pull it from API):
  ...
  return (
    <div className="space-y-5 max-w-5xl">
      ...
      {/* ── Greeting (replaced with user?.full_name) ───────────────────────── */}
      <div>
      ...
      {/* ── Stat Cards (values now derived) ────────────────────────────────── */}
      <div className="grid grid-cols-2 gap-4">
        {[
          { label: "Upcoming",     value: upcomingAppointment ? "1" : "0", sub: "appointments" },
          { label: "Total Visits", value: recentVisits.length.toString(),    sub: "completed"  },
        ].map(({ label, value, sub }) => (
          <div
            key={label}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4 items-start">
        ...
        <div className="space-y-4">
          ...
          {/* Next Appointment (uses upcomingAppointment) */}
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            ...
            {loadingData ? (
              ...
              <div className="p-5">
                ...
                {/* Doctor */}
                <p className="text-lg font-bold text-slate-800 mb-4">{upcomingAppointment.doctor}</p>
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-6 mb-5">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-slate-700">{upcomingAppointment.date}</p>
                  </div>
                  <div>
                    <p className="text-[11px] text-slate-400 flex items-center gap-1 mb-0.5">
                      <MdAccessTime className="text-[11px]" /> Time
                    </p>
                    <p className="text-sm font-semibold text-slate-700">{upcomingAppointment.time}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-[11px] text-slate-400 flex items-center gap-1 mb-0.5">
                      <MdPerson className="text-[11px]" /> Reason
                    </p>
                    <p className="text-sm font-semibold text-slate-700">{upcomingAppointment.reason}</p>
                  </div>
                </div>
          ...
          {/* Recent Visits (uses recentVisits from state) */}
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
