...
import { MdFace, MdMedicalServices, MdAccessTime, MdPerson } from "react-icons/md"
import logo from "../../assets/logo.png"


// ── Mock data ─────────────────────────────────────────────────────────────────
// In production, replace this with a fetch() to your queue API endpoint.
// The shape should be: { serving: {...} | null, waiting: [...], clinicOpen: boolean }
const MOCK_QUEUE = {
  serving: {
    queueNo: 4,
    patient: "Grace Tan",
    type: "derma",
    doctor: "Dr. Paula Gonzales-Carait",
    arrivedAt: "9:45 AM",
  },
  waiting: [
    { queueNo: 5, patient: "Marco Reyes",   type: "medical", arrivedAt: "10:00 AM" },
    { queueNo: 6, patient: "Lisa Santos",   type: "derma",   arrivedAt: "10:10 AM" },
    { queueNo: 7, patient: "Ben Cruz",      type: "medical", arrivedAt: "10:20 AM" },
    { queueNo: 8, patient: "Ana Villanueva",type: "derma",   arrivedAt: "10:35 AM" },
  ],
  clinicOpen: true,
}
...
// Estimate wait time: assume ~15 min per patient ahead
...
const QueueDisplay = () => {
  ...
  useEffect(() => {
    const poll = setInterval(() => {
      ...
      // For demo: no-op (data stays static)
    }, 10000)
    ...
    <div className="min-h-screen bg-[#0b1a2c] text-white flex flex-col select-none overflow-hidden">
      ...
      <main className="flex-1 flex gap-6 p-8">
        ...
        <div className="flex-1 flex flex-col">
          ...
          {/* Now Serving panel */}
          <div
            className={`rounded-3xl border-2 p-8 flex flex-col items-center text-center mb-6 flex-shrink-0
              ${serving
                ? "bg-sky-500/10 border-sky-400/40"
                : "bg-white/5 border-white/10"}`}
          >
            <p className="text-sky-400 text-sm font-bold uppercase tracking-[0.2em] mb-4">
              Now Serving
            </p>
          ...
          {/* Stats bar (without "Served Today") */}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <p
                          className={`text-xs ${
                            entry.type === "derma" ? "text-emerald-400" : "text-blue-400"
                          }`}
                        >
      ...
      <footer className="px-10 py-4 border-t border-white/5 flex items-center justify-between">
        ...
        <p className="text-slate-600 text-xs">
          Updates every 10 seconds · Carait Medical and Dermatology Clinic
        </p>
      </footer>
    </div>
  )
