// client/src/pages/shared/Appointments.jsx
// Single shared component for both /staff/appointments and /admin/appointments.
// Pass the correct service functions via the `services` prop.
...
import {
  ...
  MdPhone, MdEmail, MdHome, MdCake, MdWc, MdFilterList,
} from 'react-icons/md'
...
const RescheduleModal = ({ appt, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await onSave(appt.id, date, time, reason)
      onClose()
...
const AddAppointmentModal = ({ onClose, onAdd, services }) => {
  const { getPatients, getDoctors, createAppointment } = services
  ...
  useEffect(() => { getDoctors().then(d => setDoctors(Array.isArray(d) ? d : [])).catch(() => {}) }, [getDoctors])
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            ...
            {patients.length > 0 && (
              <div className="mt-1 bg-white border border-slate-200 rounded-xl max-h-36 overflow-y-auto shadow-lg relative z-10">
                {patients.map(p => (
                  <button key={p.id}
                    onClick={() => { setForm(f => ({...f, patient_id:p.id})); setPatSearch(p.full_name||p.name||''); setPatients([]) }}
                    className="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 border-b border-slate-100 last:border-0">
                    <span className="font-semibold text-slate-800">{p.full_name||p.name}</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" value={form.appointment_date} min={todayISO()}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-100 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-slate-800 truncate">{appt.patient_name||appt.patient}</p>
          <p className="text-xs text-slate-500">{appt.doctor} · {appt.specialty}</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        ...
        <div className="bg-slate-50 rounded-2xl p-4 space-y-3">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-slate-800">{appt.type==='derma'?'Dermatology':'General Medicine'}</p>
          </div>
          {appt.reason && (
            <div>
      ...
      {isActive && (
        <div className="px-6 pb-5 pt-3 border-t border-slate-100 space-y-2 shrink-0">
          {appt.status === 'pending' && (
          ...
          <div className="flex gap-2">
            <button onClick={() => onReschedule(appt)}
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold text-sky-700 bg-sky-50 border border-sky-200 hover:bg-sky-100 rounded-xl transition-colors">
              <MdRefresh className="text-[14px]" /> Reschedule
            </button>
            <button onClick={() => onCancel(appt.id)}
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-bold text-red-600 bg-red-50 border border-red-200 hover:bg-red-100 rounded-xl transition-colors">
...
// ─── Main Shared Appointments ─────────────────────────────────────────────────
const Appointments = ({ services }) => {
  const { getAppointments, confirmAppointment, cancelAppointment, rescheduleAppointment } = services
  ...
  const load = useCallback((todayFilter) => {
    setLoading(true)
    const params = todayFilter ? `?date=${todayISO()}` : ''
  ...
  }, [getAppointments]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => { load(todayOnly) }, [todayOnly]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleRescheduleSave = async (id, date, time, reason) => {
    try {
      await rescheduleAppointment(id, { appointment_date: date, appointment_time: time, reason })
  ...
  return (
    <div className="max-w-5xl space-y-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold px-4 py-2.5 rounded-xl border transition-colors
              ${todayOnly ? 'bg-sky-600 text-white border-sky-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
            <MdCalendarToday className="text-[14px]" /> {todayOnly ? 'Today ✓' : 'Today'}
          </button>
          <button onClick={() => load(todayOnly)} title="Refresh"
      ...
      {todayOnly && (
        <div className="bg-sky-50 border border-sky-200 rounded-xl px-4 py-2.5 flex items-center gap-2">
          <MdFilterList className="text-sky-500 text-[14px] shrink-0" />
          <p className="text-xs text-sky-700 font-medium">
            Showing today only — <button onClick={() => setTodayOnly(false)} className="underline font-semibold">Show all</button>
          </p>
        </div>
      )}
      ...
      {showAdd && (
        <AddAppointmentModal onClose={() => setShowAdd(false)} onAdd={handleAdd} services={services} />
      )}
    </div>
  )
}
