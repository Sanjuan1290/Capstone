...
const StaffWalkInQueue = () => {
  ...
  const active=queue.filter((i)=>['waiting','called','in_consultation'].includes(i.status)),completed=queue.filter((i)=>['done','removed'].includes(i.status));const activePg=useClientPagination(active,{initialPageSize:6}),donePg=useClientPagination(completed,{initialPageSize:6})
