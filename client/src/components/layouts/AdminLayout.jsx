...
const sideNav = [
  ...
  { name: "Staff Accounts",   path: "/admin/staff-accounts",    icon: MdPeople           },
  { name: "Doctor Accounts",  path: "/admin/doctor-accounts",   icon: MdMedicalServices  },
  { name: "Doctor Schedules", path: "/admin/doctor-schedules",  icon: MdSchedule         },
  { name: "Appointments",     path: "/admin/appointments",      icon: MdEventAvailable   },
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      <aside className={`relative flex flex-col bg-[#0b1a2c] shrink-0
        transition-[width] duration-300 ease-in-out ${collapsed ? "w-[72px]" : "w-60"}`}>

        {/* Brand */}
        <div className="flex items-center gap-3 px-4 h-16 border-b border-white/5 overflow-hidden">
          <img src={logo} alt="Carait Clinic" className="w-9 h-9 rounded-xl object-contain bg-white/10 p-1 shrink-0" />
          <div className={`leading-tight overflow-hidden whitespace-nowrap transition-[opacity,max-width] duration-300
            ...
            <span className="text-amber-400 text-[10px] font-semibold uppercase tracking-widest font-mono">Admin</span>
          </div>
        </div>
            ...
            <NavLink key={path} to={path} end={path === "/admin"}
              ...
              {({ isActive }) => (<>
                ...
                <span className={`whitespace-nowrap overflow-hidden transition-[opacity,max-width] duration-300
                  ${collapsed ? "opacity-0 max-w-0" : "opacity-100 max-w-xs"}`}>{name}</span>
                {collapsed && (
                  <span className="absolute left-full ml-3 px-2.5 py-1.5 bg-slate-800 text-white text-xs
                    rounded-lg whitespace-nowrap shadow-lg border border-white/10 opacity-0
                    pointer-events-none group-hover:opacity-100 transition-opacity duration-150 z-50">{name}</span>
                )}
              </>)}
            </NavLink>
          ))}
        ...
        <div className="px-3 py-4 border-t border-white/5 space-y-1">
          {logoutError && !collapsed && <p className="text-[10px] text-red-400 px-3 pb-1">{logoutError}</p>}
          <button onClick={handleLogout} disabled={loggingOut}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium w-full
              text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-colors disabled:opacity-50">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-6 shrink-0">
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2
            ...
            <MdSearch className="text-[15px]" /><span>Search…</span>
          </div>
